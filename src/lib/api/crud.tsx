...
const ROUTING_URL: string = (process.env.NEXT_PUBLIC_BACKEND_URL || '/api/') as string;
...
const refreshAccessToken = async () => {
  try {
    ...
    const endpoint = getRoute("REFRESH_TOKEN");
...
export const fetchData = async (
  ...
  const baseUrl = ROUTING_URL.endsWith('/') ? ROUTING_URL : `${ROUTING_URL}/`;
  const endpoint = pipe(getRoute(key), params);
  const cleanEndpoint = endpoint.startsWith('/') ? endpoint.substring(1) : endpoint;
